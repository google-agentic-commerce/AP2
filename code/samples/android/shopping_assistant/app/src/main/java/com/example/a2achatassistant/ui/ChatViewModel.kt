...
class ChatViewModel(application: Application) : AndroidViewModel(application) {
  ...
  fun sendMessage(userMessage: String, activity: Activity?) {
    ...
    viewModelScope.launch {
      ...
      result
        .onSuccess { response ->
          Log.d(TAG, "Repository returned success: '${response.text}'")
          val geminiMessage =
            ChatMessage(
              text = response.text,
              sender = SenderRole.GEMINI,
              collapsedTitle =
                if (response.collapsedData != null) "Raw dSD-JWT Presentations" else null,
              collapsedData = response.collapsedData,
            )
