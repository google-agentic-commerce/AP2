#!/bin/bash
# ---------------------------------------------------------------------------
# Run all servers for the A2A human-not-present flow (x402) and open the web client.
#
# Prerequisites: Node.js (npm), uv (astral.sh), curl
# Usage:         ./run.sh [--enable_broadcast_on_chain]   (run from anywhere)
# ---------------------------------------------------------------------------
...
ENABLE_BROADCAST=""
for arg in "$@"; do
  if [ "$arg" = "--enable_broadcast_on_chain" ]; then
...
readonly X402_PSP_TRIGGER_PORT=8084
mkdir -p "$LOG_DIR"
...
# Apply flag or fallback
if [ "$ENABLE_BROADCAST" = "TRUE" ]; then
  export BROADCAST_ON_CHAIN="TRUE"
else
  export BROADCAST_ON_CHAIN="${BROADCAST_ON_CHAIN:-FALSE}"
fi
...
export MERCHANT_PAYMENT_PROCESSOR_SIGNING_KEY_PATH="$TEMP_DB_DIR/merchant_payment_processor_signing_key.pem"
...
kill_port $X402_PSP_TRIGGER_PORT
start_service "x402-psp-trigger" "../../../../src/roles/x402_psp_mcp" \
  "uv run python trigger_server.py" $X402_PSP_TRIGGER_PORT
sleep 1
...
start_service "web-client" "../../../../../../web-client" \
  "VITE_FLOW=x402 npm run dev -- --port $WEB_CLIENT_PORT" $WEB_CLIENT_PORT
