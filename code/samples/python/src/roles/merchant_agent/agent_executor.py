...
class MerchantAgentExecutor(BaseServerExecutor):
  ...
  _system_prompt = """
    ...
    You can find items, update shopping carts, initiate payments, and finalize
    Digital Payment Credential (DPC) flows.

    When you receive a 'checkout_mandate', you MUST use the 'dpc_finish' tool
  ...
  async def _handle_request(
    ...
    if not await self._validate_shopping_agent(data_parts, updater):
      error_message = updater.new_agent_message(
          parts=[Part(root=TextPart(text="Failed to validate shopping agent."))]
      )
  ...
  async def _validate_shopping_agent(
    ...
    Args:
      ...
      updater: The task updater for failing the task on error.
