...
Reads vectors-v0.json, recomputes JCS(RFC 8785)(mandate_body) + SHA-256 for
each vector, and verifies:
  ...
  4. for vectors carrying jws_compact: the base64url-decoded JWS payload,
     parsed and re-canonicalized, produces the same open_mandate_hash
     (envelope invariance; signatures are not verified)
...
def jws_payload_hash(jws_compact: str) -> str:
    """Return sha256 hex of the canonicalized claims parsed from a JWS payload."""
    payload_b64 = jws_compact.split(".")[1]
    payload = base64.urlsafe_b64decode(payload_b64 + "=" * (-len(payload_b64) % 4))
    claims = json.loads(payload.decode("utf-8"))
    return hashlib.sha256(rfc8785.dumps(claims)).hexdigest()


def main() -> int:
    if len(sys.argv) < 2:
        print("usage: runner_python.py vectors-v0.json", file=sys.stderr)
        return 2
    ...
    for v in vectors:
        ...
        jws_ok = True
        if "jws_compact" in v:
            jws_ok = jws_payload_hash(v["jws_compact"]) == sha
        ok = bytes_ok and sha_ok and jws_ok
        ...
        if not ok:
            ...
            if not jws_ok:
                print(f"        jws payload hash mismatch (envelope invariance broken)")
        pass_ += int(ok)
        fail += int(not ok)
