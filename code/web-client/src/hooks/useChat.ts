...
import {
  ...
  extractInventoryOptionsFromText,
  extractMandateFromText,
  extractMonitoringFromText,
  extractMonitoringJsonFromText,
  extractProductPreviewUnavailableFromText,
  extractPurchaseCompleteFromText,
  parseInvocationParts,
  parseMainArtifactData,
  parseToolAndInventoryArtifacts,
} from "../utils/parsing";
...
export function useChat() {
  ...
  const [lastSelectedItemName, setLastSelectedItemName] = useState<
    string | undefined
  >();
  const [lastInventoryMatches, setLastInventoryMatches] = useState<
    InventoryMatch[] | undefined
  >();
  const [lastInventoryOptions, setLastInventoryOptions] = useState<
  ...
  const sendToAgent = useCallback(
    async (text: string | OutgoingDataPayload, taskId?: string) => {
      setLoading(true);
        ...
        for await (const event of a2aClient.sendMessage(text, tid)) {
          if (event.type === "status") {
                ...
                if (toolName === "assemble_and_sign_mandates_tool") {
                  if (
                    ...
                    Promise.all([
                      ...
                      .catch((e) =>
                        console.error("Failed to fetch open mandates:", e)
                      );
                  }
                } else if (
                  ...
                  fetchMandate(resp.checkout_mandate_chain_id)
                    ...
                    .catch((e) =>
                      console.error("Failed to fetch checkout mandate:", e)
                    );
                } else if (
                  ...
                  fetchMandate(resp.payment_mandate_chain_id)
                    ...
                    .catch((e) =>
                      console.error("Failed to fetch payment mandate:", e)
                    );
                }
              }
            }
            ...
            if (statusParts.length > 0) {
              const strictStatusParts = statusParts
                .map((p) => convertToStrictPart(p))
            ...
            if (!event.artifact.lastChunk && agentTextBuffer) {
              ...
              if (earlyMon) {
                setMessages((prev) =>
                  upsertMonitoringMessage(prev, earlyMon, agentTextBuffer)
                );
              }
            }
            ...
            if (event.artifact.lastChunk && agentTextBuffer) {
              if (inventoryOpts.length === 0) {
                ...
                if (inv) {
                  ...
                  const sel = inv.matches.find(
                    (m) => m.item_id === inv.selected
                  );
                  if (sel) setLastSelectedItemName(sel.name);
                  setLastInventoryMatches(inv.matches);
                  setLastInventoryOptions(inv);
            ...
            if (event.artifact.lastChunk) {
              const strictParts = parts
                .map((p) => convertToStrictPart(p))
                .filter((p): p is Part => p !== undefined);
              const mainData =
                parseMainArtifactData(strictParts) ??
                (agentTextBuffer
                  ? (extractMandateFromText(agentTextBuffer) ??
                    extractProductPreviewUnavailableFromText(agentTextBuffer) ??
    ...
    const interval = setInterval(async () => {
      try {
        const resp = await fetch(
          `${MERCHANT_TRIGGER_URL}/state?item_id=${encodeURIComponent(
        ...
        if (
          str !== lastTriggerStateRef.current &&
  ...
  useEffect(() => {
    if (
      !loading &&
      pendingTriggerNudgeRef.current &&
      monitoringData?.item_id &&
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (!loadingRef.current) {
        const msg =
          monitoringData?.item_id != null && monitoringData?.price_cap != null
            ? {
  ...
  async function handleMandateApprove(mandateRequest: MandateApprovalData) {
    addMessage({
      ...
      userActionSublabel:
        "User signed over the TS surface with agent provider key",
    });
    await sendToAgent(
  ...
  function handleMandateReject() {
    addMessage({
      role: "system",
      text: "Mandate rejected. Purchase cancelled.",
    });
  }
