...
class EnhancedMerchantAgentTools:
    ...
    async def initiate_payment_enhanced(
        ...
        if not payment_mandate_data:
            ...
            await self._fail_task_enhanced(
                updater, error_response, session_id=session_id
            )
            return
        ...
        try:
            ...
            payment_mandate = PaymentMandate(
                PaymentMandateContents(
                    payment_mandate_id=payment_mandate_data[
                        'payment_mandate_id'
                    ],  # Direct access - let KeyError be raised
                    payment_details_total=payment_mandate_data.get(
            ...
            error_response = self.error_handler.create_error_response(
                ...
                message=f'Missing required field in payment mandate: {e!s}',
                details={
                    'missing_field': str(e),
                    'received_data_keys': list(payment_mandate_data.keys())
                    if payment_mandate_data
                    else [],
            ...
            error_response = self.error_handler.create_error_response(
                ...
                severity=ErrorSeverity.HIGH,
                message=f'Invalid payment mandate format: {e!s}',
                details={
        ...
        payment_amount = (
            payment_mandate.payment_mandate_contents.payment_details_total.get(
                'value', 0
            )
        )
        validation_error = self._validate_payment_constraints(
            mandate_id, payment_amount, session_id
        )
        if validation_error:
    ...
    def _validate_payment_constraints(
        self, mandate_id: str, payment_amount: float, session_id: str
    ) -> dict[str, Any] | None:
...
class BackwardCompatibleMerchantAgent:
    ...
    async def initiate_payment(
        ...
        try:
            ...
            mandate_id = payment_mandate_data.get(
                'payment_mandate_id', 'unknown'
            )
            self.audit_logger.log_mandate_event(
                event_category='mandate_execution',
            ...
            error_response = self.error_handler.create_error_response(
                ...
                severity='high',
                message=f'Unexpected error during payment processing: {e!s}',
            )
...
async def demo_agent_integration():
    ...
    updater = TaskUpdater()
    await enhanced_agent.initiate_payment_enhanced(
        payment_data, updater, debug_mode=True
    )
    ...
    await enhanced_agent.initiate_payment_enhanced(
        violation_data, violation_updater
    )
    print(
        f'Violation test completed with {len(violation_updater.messages)} messages'
    )
    ...
    print(
        f'Compatible agent completed with {len(compat_updater.messages)} messages'
    )
